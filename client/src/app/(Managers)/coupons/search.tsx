...
export type CouponSearchState = {
  keyword: string
  isActive: '' | 'true' | 'false'
  discountType: string
}

type Props = { expanded: boolean; onToggle: () => void; form: CouponSearchState; onFormChange: (n: CouponSearchState) => void; onSearch: () => void; onReset: () => void }
const primaryBtn = 'bg-[#1677ff] hover:bg-[#0958d9] border-[#1677ff] font-bold uppercase tracking-widest text-[10px] h-10'

export default function CouponSearch({ expanded, onToggle, form, onFormChange, onSearch, onReset }: Props) {
  return (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2.5">
                <div className="p-1 rounded-sm bg-blue-50 text-[#1677ff]"><FileTextOutlined /></div>Mã / Mô tả
              </label>
              <Input allowClear placeholder="Nhập mã coupon…" value={form.keyword} className="rounded-sm h-10"
                onChange={(e) => onFormChange({ ...form, keyword: e.target.value })} />
            </div>
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2.5">
                <div className="p-1 rounded-sm bg-blue-50 text-[#1677ff]"><TagOutlined /></div>Loại giảm
              </label>
              <Select className="w-full h-10" placeholder="Tất cả" value={form.discountType || undefined} allowClear
                options={[{ value: '0', label: 'Giảm %' }, { value: '1', label: 'Giảm tiền' }, { value: '2', label: 'Miễn ship' }]}
                onChange={(v) => onFormChange({ ...form, discountType: v ?? '' })} />
            </div>
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2.5">
            ...
            <div className="md:col-span-3 flex items-end gap-3 lg:justify-end">
