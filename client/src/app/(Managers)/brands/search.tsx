...
type Props = { expanded: boolean; onToggle: () => void; form: BrandSearchState; onFormChange: (n: BrandSearchState) => void; onSearch: () => void; onReset: () => void }
const primaryBtn = 'bg-[#1677ff] hover:bg-[#0958d9] border-[#1677ff] font-bold uppercase tracking-widest text-[10px] h-10'

export default function BrandSearch({ expanded, onToggle, form, onFormChange, onSearch, onReset }: Props) {
  return (
        ...
        <div className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
            <div className="space-y-2 md:col-span-2">
              ...
              <Input allowClear placeholder="Tên thương hiệu, slug…" value={form.keyword} className="rounded-sm h-10" onChange={(e) => onFormChange({ ...form, keyword: e.target.value })} />
            </div>
