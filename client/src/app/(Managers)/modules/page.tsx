...
import { rbacAdminApi } from '@/services/rbacAdminApi'
import ModuleSearchPanel, { type ModuleSearchState } from './search'
import ModuleCreateOrUpdate, { type ModuleModalMode } from './createOrUpdate'

const primaryBtn =
  'bg-[#1677ff] hover:bg-[#0958d9] border-[#1677ff] font-bold uppercase tracking-widest'

function matchesFilter(row: ModuleDto, q: ModuleSearchState): boolean {
  const kw = q.keyword.trim().toLowerCase()
  if (kw) {
    const ok =
      row.name.toLowerCase().includes(kw) ||
      (row.code?.toLowerCase().includes(kw) ?? false) ||
      (row.link?.toLowerCase().includes(kw) ?? false)
    if (!ok) return false
  }
...
export default function ModuleManagementPage() {
  ...
  const [applied, setApplied] = useState<ModuleSearchState>({
    keyword: '',
    status: '',
  })
  const [draft, setDraft] = useState<ModuleSearchState>({
    keyword: '',
    status: '',
  })
  ...
  const [modalMode, setModalMode] = useState<ModuleModalMode>('create')
  const [selected, setSelected] = useState<ModuleDto | null>(null)
  ...
  const handleReset = () => {
    const empty: ModuleSearchState = { keyword: '', status: '' }
    setDraft(empty)
    setApplied(empty)
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 mt-1.5 text-sm font-medium">
              Quản lý module hiển thị trong hệ thống
            </p>
          </div>
        </div>
        <Button
